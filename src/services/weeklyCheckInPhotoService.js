import { supabase } from '../lib/supabase'

const PHOTO_FIELDS = `
  id,
  user_id,
  coaching_plan_id,
  weekly_checkin_id,
  photo_context,
  pose,
  side_view,
  storage_path,
  mime_type,
  size_bytes,
  created_at,
  updated_at
`

const MIME_EXTENSION_MAP = {
  'image/jpeg': 'jpg',
  'image/png': 'png',
  'image/webp': 'webp',
  'image/heic': 'heic',
  'image/heif': 'heif',
}

const ALLOWED_POSES = [
  'front',
  'side',
  'back',
]

function getPhotoExtension(file) {
  const mimeExtension =
    MIME_EXTENSION_MAP[file?.type]

  if (mimeExtension) {
    return mimeExtension
  }

  const filenameExtension = String(
    file?.name ?? '',
  )
    .split('.')
    .pop()
    .toLowerCase()

  if (
    Object.values(
      MIME_EXTENSION_MAP,
    ).includes(filenameExtension)
  ) {
    return filenameExtension
  }

  throw new Error(
    'Use a JPEG, PNG, WebP, HEIC, or HEIF image.',
  )
}

async function getSignedInUserId() {
  const {
    data: { user },
    error,
  } = await supabase.auth.getUser()

  if (error) {
    throw error
  }

  if (!user) {
    throw new Error(
      'You must be signed in to upload photos.',
    )
  }

  return user.id
}

async function addSignedUrl(photo) {
  const { data, error } =
    await supabase.storage
      .from('progress-photos')
      .createSignedUrl(
        photo.storage_path,
        60 * 60,
      )

  if (error) {
    throw error
  }

  return {
    ...photo,
    signed_url: data.signedUrl,
  }
}

export async function loadWeeklyCheckInPhotos(
  weeklyCheckInId,
) {
  if (!weeklyCheckInId) {
    return []
  }

  const { data: photos, error } =
    await supabase
      .from('progress_photos')
      .select(PHOTO_FIELDS)
      .eq(
        'weekly_checkin_id',
        weeklyCheckInId,
      )
      .order('created_at')

  if (error) {
    throw error
  }

  return Promise.all(
    (photos ?? []).map(addSignedUrl),
  )
}

export async function uploadWeeklyCheckInPhoto({
  coachingPlanId,
  weeklyCheckInId,
  pose,
  sideView = null,
  file,
}) {
  if (
    !coachingPlanId ||
    !weeklyCheckInId ||
    !file
  ) {
    throw new Error(
      'A plan, Weekly Check-In, and photo are required.',
    )
  }

  if (!ALLOWED_POSES.includes(pose)) {
    throw new Error('The photo pose is invalid.')
  }

  if (
    pose === 'side' &&
    !['left', 'right'].includes(sideView)
  ) {
    throw new Error(
      'Use the saved left or right side photo.',
    )
  }

  const userId =
    await getSignedInUserId()
  const extension =
    getPhotoExtension(file)

  const storagePath = [
    userId,
    coachingPlanId,
    'weekly',
    weeklyCheckInId,
    `${pose}-${Date.now()}.${extension}`,
  ].join('/')

  const {
    data: existing,
    error: loadError,
  } = await supabase
    .from('progress_photos')
    .select(PHOTO_FIELDS)
    .eq(
      'weekly_checkin_id',
      weeklyCheckInId,
    )
    .eq('pose', pose)
    .maybeSingle()

  if (loadError) {
    throw loadError
  }

  const { error: uploadError } =
    await supabase.storage
      .from('progress-photos')
      .upload(storagePath, file, {
        cacheControl: '3600',
        contentType:
          file.type || undefined,
        upsert: false,
      })

  if (uploadError) {
    throw uploadError
  }

  const photoValues = {
    user_id: userId,
    coaching_plan_id:
      coachingPlanId,
    start_checkin_id: null,
    weekly_checkin_id:
      weeklyCheckInId,
    photo_context: 'weekly',
    pose,
    side_view:
      pose === 'side'
        ? sideView
        : null,
    storage_path: storagePath,
    mime_type: file.type || null,
    size_bytes: file.size || null,
  }

  const photoQuery = existing
    ? supabase
        .from('progress_photos')
        .update(photoValues)
        .eq('id', existing.id)
    : supabase
        .from('progress_photos')
        .insert(photoValues)

  const {
    data: savedPhoto,
    error: metadataError,
  } = await photoQuery
    .select(PHOTO_FIELDS)
    .single()

  if (metadataError) {
    await supabase.storage
      .from('progress-photos')
      .remove([storagePath])

    throw metadataError
  }

  if (
    existing?.storage_path &&
    existing.storage_path !== storagePath
  ) {
    const { error: removeError } =
      await supabase.storage
        .from('progress-photos')
        .remove([
          existing.storage_path,
        ])

    if (removeError && import.meta.env.DEV) {
      console.debug(
        '[weeklyCheckInPhotoService] Old photo object could not be removed.',
        removeError,
      )
    }
  }

  return {
    ...(await addSignedUrl(savedPhoto)),
    name: file.name,
  }
}
